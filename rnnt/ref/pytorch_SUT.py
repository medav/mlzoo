...
import sys
import os
sys.path.insert(0, os.path.join(os.getcwd(), "rnnt/ref"))
...
class PytorchSUT:
    def __init__(self, config_toml, checkpoint_path):
        config = toml.load(config_toml)
        self.labels = config['labels']['labels']
        ...
        model.load_state_dict(
            load_and_migrate_checkpoint(checkpoint_path), strict=True)
        model.eval()
