...
class LstmDrop(torch.nn.Module):
    ...
    def __init__(self, config : Config):
        ...
        if config.forget_bias is not None:
            for name, v in self.lstm.named_parameters():
                if "bias_ih" in name:
                    bias = getattr(self.lstm, name)
                    bias.data[config.h_dim:2 * config.h_dim] \
                        .fill_(config.forget_bias)
                if "bias_hh" in name:
                    bias = getattr(self.lstm, name)
                    bias.data[config.h_dim:2 * config.h_dim].fill_(0)

        self.drop = torch.nn.Dropout(config.dropout, inplace=True)
...
class StackTime(torch.nn.Module):
    ...
    def forward(self, x : torch.Tensor, x_lens : torch.Tensor):
        ...
        [B, T, U] = r.shape
        zeros = torch.zeros(
            B, (-T) % self.factor, U, dtype=r.dtype, device=r.device)
...
class Encoder(torch.nn.Module):
    ...
    class Config:
        ...
        h_dim : int
        pre_rnn_nlayers : int
        post_rnn_nlayers : int
        forget_bias : float
        stack_time_factor : int
        dropout : float
    ...
    def __init__(self, config : Config):
        ...
        self.pre_lstms = LstmDrop(
            LstmDrop.Config(
                config.in_dim,
                config.h_dim,
                config.pre_rnn_nlayers,
                config.dropout,
                config.forget_bias))

        self.stack_time = StackTime(factor=config.stack_time_factor)

        self.post_lstms = LstmDrop(
            LstmDrop.Config(
                config.stack_time_factor * config.h_dim,
                config.h_dim,
                config.post_rnn_nlayers,
                config.dropout,
                config.forget_bias))

    def forward(self, x_padded : torch.Tensor, x_lens : torch.Tensor):
...
class Prediction(torch.nn.Module):
    def __init__(
        ...
        self.dec_rnn = LstmDrop(
            LstmDrop.Config(
                n_hidden, n_hidden, pred_rnn_layers, dropout, forget_gate_bias))
...
class Rnnt(torch.nn.Module):
    ...
    def _infer(self, x_padded: torch.Tensor, x_lens: torch.Tensor):
        ...
        for batch_idx in range(enc_logits.size(0)):
            ...
            sentence = ''.join([self.labels[i] for i in sentence])
